...
import {  usePutApiMyVmByProjectIdNetworkNodeEnablePortSecurityAndIdMutation } from "src/app/services/api.generated";
...
export const EnablePortDialog: FC<EnablePortDialogPropsType> = ({
  ...
  const submit = () => {
    if (!id) return;
        enableport({ projectId: Number(projectId), id })
          .unwrap()
          .then(() => {
      ...
      <Stack p={{ xs: 1.8, md: 3 }} spacing={{ xs: 2, md: 5 }}>
        <Stack>
          <Typography variant="text1" color="error" fontWeight="bold">
            آیا از فعال سازی port security  مطمئن هستید؟
          </Typography>
