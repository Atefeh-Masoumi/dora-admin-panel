...
type CreateVolumeSnapshotDialogPropsType = DialogProps & {
  blockstorageId: number;
  forceClose: () => void;
  refetch: () => void;
};

export const CreateVolumeSnapshotDialog: FC<CreateVolumeSnapshotDialogPropsType> = ({
  blockstorageId,
  forceClose,
  refetch,
  ...props
  ...
  const onSubmit: formikOnSubmitType<typeof initialValues> = (
    ...
    createSnapshot({
      ...
      .unwrap()
      .then(() => {
        toast.success("اسنپ شات دیسک ابری با موفقیت ایجاد شد");
        forceClose();
        refetch();
      })
      .catch((_err: unknown) => {})
  ...
  return (
    <Dialog {...props}>
      <DialogTitle align="center">ایجاد اسنپ شات دیسک ابری</DialogTitle>
