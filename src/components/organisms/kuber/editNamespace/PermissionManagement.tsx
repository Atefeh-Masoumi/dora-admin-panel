...
import { permissionTableStruct } from "../workSpacePermissions/tables/struct";
import { PermissionTableRow } from "../workSpacePermissions/tables/PermissionTableRow";
import { AddPermissionDialog } from "../workSpacePermissions/dialogs/AddPermissionDialog";

type PermissionManagementPropsType = { row: any };

type PermissionManagementContextValueType = {
  refetchUsersData: () => any;
};

export const PermissionManagementContext =
  createContext<PermissionManagementContextValueType>({
    refetchUsersData: () => null,
  });

export const PermissionManagement: FC<PermissionManagementPropsType> = ({
  row,
}) => {
  ...
  return (
    ...
    >
        ...
        <Grid2 xs={12} md={8}>
          ...
          >
            ...
            <Box width="100%" sx={{ pt: 1.5 }}>
              <BaseTable
                struct={permissionTableStruct}
                RowComponent={PermissionTableRow}
                rows={data || []}
      ...
      {showDialog && <AddPermissionDialog hostId={row} onClose={closeDialog} />}
    </PermissionManagementContext.Provider>
  );
};
