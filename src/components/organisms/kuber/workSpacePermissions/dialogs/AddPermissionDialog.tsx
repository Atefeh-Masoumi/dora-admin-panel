...
import { PermissionManagementContext } from "../../editNamespace/PermissionManagement";

const formInitialValues = {
  selectedUser: 0,
};

type AddPermissionDialogPropsType = {
  onClose: () => void;
  hostId: number;
};

export const AddPermissionDialog: FC<AddPermissionDialogPropsType> = ({
  onClose,
  hostId,
}) => {
  const { refetchUsersData } = useContext(PermissionManagementContext);

  const { data: userList } = useGetPortalKubeUserShortListQuery();

  const [
    createUserPermissionCreate,
    { isLoading: createUserPermissionLoading },
  ] = usePostPortalKubeUserRoleCreateMutation();
              ...
              <Stack p={{ xs: 1.8, md: 3 }} spacing={{ xs: 2, md: 5 }}>
                ...
                <DialogActions>
                  ...
                  <LoadingButton
                    component="button"
                    type="submit"
                    loading={createUserPermissionLoading}
                    variant="contained"
                    sx={{ px: 3, py: 0.8 }}
                  >
                    ذخیره تغییرات
