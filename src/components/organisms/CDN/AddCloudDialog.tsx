...
type AddCloudDialogPropsType = {
  open: boolean;
  onClose: () => void;
  loadBalance?: EditLoadBalanceModel;
};

export const AddCloudDialog: FC<AddCloudDialogPropsType> = ({
  open,
  onClose,
  loadBalance,
}) => {
  ...
  > = {
    ...
    loadBalancingPolicyId: loadBalance?.loadBalancingPolicyId || 1,
  ...
  const [destinations, setDestinations] = useState<DestinationModel[]>(
    (loadBalance?.destinations as any) || []
  );
  const [certificateSwitch, setCertificateSwitch] = useState(
    loadBalance?.dangerousAcceptAnyServerCertificate || false
  );
