...
import { useGetApiV2RabbitRabbitHostGetByIdQuery } from "src/app/services/api.generated";
...
export const ServiceInfo: FC<ServiceInfoPropsType> = () => {
  ...
  useEffect(() => {
    if (rabbitData?.datacenterId) {
      setDataCenter(rabbitData.datacenterId);
    }
  }, [setDataCenter, rabbitData]);

  const isActive = useMemo(() => rabbitData?.statusId === 2, [rabbitData?.statusId]);
