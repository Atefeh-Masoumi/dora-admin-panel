...
import {  useGetApiMyVmByProjectIdSnapshotListQuery, usePutApiMyVmByProjectIdSnapshotRevertAndIdMutation } from "src/app/services/api.generated";
import { toast } from "react-toastify";
import { LoadingButton } from "@mui/lab";
import { useNavigate, useParams } from "react-router";
import { BORDER_RADIUS_1 } from "src/configs/theme";

type RevertSnapshotDialogPropsType = {
  openDialog: boolean;
  handleClose: () => void;
  snapshotId: number;
};

export const RevertSnapshotDialog: FC<RevertSnapshotDialogPropsType> = ({
  openDialog,
  handleClose,
  snapshotId,
}) => {
  ...
  const submit = () => {
    ...
    revertSnapshot({
      ...
      .unwrap()
      .then(() => {
        toast.success("فرآیند بازگردانی به اسنپ شات مورد نظر با موفقیت آغاز شد");
        handleClose();
       refetch();
      })
      .catch((_err: unknown) => {});
